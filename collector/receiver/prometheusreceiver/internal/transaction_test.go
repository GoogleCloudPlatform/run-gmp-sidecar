...
func newNopSettings() receiver.Settings {
	return receivertest.NewNopSettings(component.MustNewType("prometheus"))
}
...
func TestTransactionCommitWithoutAdding(t *testing.T) {
	tr := newTransaction(scrapeCtx, &startTimeAdjuster{startTime: startTimestamp}, consumertest.NewNop(), labels.EmptyLabels(), newNopSettings(), nopObsRecv(t), false)
...
func TestTransactionAppendResource(t *testing.T) {
	...
	tr := newTransaction(scrapeCtx, &startTimeAdjuster{startTime: startTimestamp}, sink, labels.EmptyLabels(), newNopSettings(), nopObsRecv(t), false)
	_, err := tr.Append(0, labels.FromMap(map[string]string{
		model.InstanceLabel:   "localhost:8080",
		model.JobLabel:        "test",
...
func TestTransactionCommitErrorWhenAdjusterError(t *testing.T) {
	...
	tr := newTransaction(scrapeCtx, &errorAdjuster{err: adjusterErr}, sink, labels.EmptyLabels(), newNopSettings(), nopObsRecv(t), false)
...
func nopObsRecv(t *testing.T) *receiverhelper.ObsReport {
	obsrecv, err := receiverhelper.NewObsReport(receiverhelper.ObsReportSettings{
		...
		Transport:              transport,
		ReceiverCreateSettings: newNopSettings(),
	})
